****************************************************************
               Welcome to SQLAPI++ Library 5.4.0
****************************************************************
...
Version 5.4.0
31 August 2025
-- OLEDB: Added OLEDB drivers support with new SA_OLEDB_Client (thanks Vincent Thomassin-Rochon).
-- MySQL/MariaDB: added build option SA_MYSQL_CACHE_KILL_CRED to cache the credentials used with SACommand::Cancel to run KILL command (thanks Frank Hilliger).
-- General: Make SACommand::UnExecute method pulic (thanks Heribert Scharnagl).
-- MySQL/MariaDB: Fixed long/LOB default max read chunk size (thanks Vincent Thomassin-Rochon).
-- SQLServer(ODBC): Support for APPNAME option (thanks Fernand Alcatrao).

Version 5.3.7
12 May 2025
-- General: SANumeric optimization (thanks Thomas Oswald).
-- General: Fixed (again) calculation of input buffer length for long/LOB parameters (thanks to Christian Lexer).

Version 5.3.6
9 April 2025
-- DuckDB: Fixed prepared statement error processing (thanks Christian Schmitz).
-- General: Fixed calculation of input buffer length for long/LOB parameters (thanks to Boris Daniel).
-- General: SQLAPI_DATETIME_WRONG_EXCEPTION build option supported in SADateTime(const SAChar*) (thanks Christian Schmitz).
-- SQL Anywhere: Implemented ::isAlive method (thanks Christian Schmitz).
-- General: Code cleanups (thanks Christian Schmitz).
