...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		*/
		...
		//con.setOption(_TSA("ODBCUseBigint")) = _TSA("1");
		//con.Connect(_TSA("MSAccess"), _TSA(""), _TSA(""), SA_ODBC_Client);
		//con.Connect(_TSA("server=bedlam"), _TSA("sa"), _TSA("java1970"), SA_SQLAnywhere_Client);
		//con.Connect(_TSA("localhost/XEPDB1"), _TSA("yas"), _TSA("java"), SA_Oracle_Client);
		//con.Connect(_TSA("localhost@test"), _TSA("postgres"), _TSA("java"), SA_PostgreSQL_Client);
		//con.setOption(_TSA("OLEDBProvider")) = _TSA("Microsoft.ACE.OLEDB.16.0");
		...
		cmd.Param(1).setLongOrLobReaderMode(SA_LongOrLobReaderManual);
		cmd.Param(1).setAsBLob(FromFileWriter, 0x1FFFFF, "c:\\src\\in.bin");
		cmd.Execute();
		*/

		cmd.setCommandText(_TSA("select f2 from t1 where id=:1"));
		cmd.Param(1).setAsInt32() = 1;
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			if (cmd.isResultSet())
			{
				...
				for (int j = 0; j < 3 && cmd.FetchNext(); ++j)
				{
					printf("Fetched %d - %d = %s\n", i + 1, j + 1, cmd.Field(1).asString().Left(20).GetMultiByteChars());
					cmd.Cancel();
				}
